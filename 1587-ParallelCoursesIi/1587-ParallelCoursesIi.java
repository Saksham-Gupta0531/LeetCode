// Last updated: 2/4/2026, 12:29:28 PM
class Solution {
    List<Integer> dp;
    ...
    public int solve(int mask, List<Integer> indegree, List<List<Integer>> adj, int k) {
        if (mask == 0) {
            return 0;
        }
        if (dp.get(mask) != -1) {
    ...
    public void generateCombinations(List<Integer> nodes, int k, int startIndex, List<Integer> currentCombination, List<List<Integer>> result) {
        if (k == 0) {
    ...
    public List<List<Integer>> uniqueCombinations(List<Integer> nodes, int k) {
